...
// Endpoint to handle status-info
app.get('/status-info', (req, res) => {
  const code = parseInt(req.query.code); // Get the status code from query parameter

  if (statusCodes[code]) {
    // If valid status code, return the status and its message
    res.json({
      status: code,
      description: statusCodes[code]
    });
  } else {
    // If invalid status code, return an error
    res.status(400).json({
      status: 400,
      description: "Bad Request: Invalid or missing status code. Please provide a valid code."
    });
  }
});
